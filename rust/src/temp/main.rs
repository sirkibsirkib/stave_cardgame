...
impl State {
    ...
    fn offset_pt_for_text(col:Col, pt: &mut Point2) {
    	match col {
    		Col::Red => (),
    		Col::Green => pt.x += 40.0,
    		Col::Blue => pt.y += 25.0,
    ...
    fn slide_card(&mut self, card_addr:CardAddr, rightward:bool) {
    	let src_col = self.staves[card_addr.stave_id].col().expect("SRC COL");
    	...
    	let dest_id = if rightward {card_addr.stave_id+1} else {card_addr.stave_id-1};
    	let dest_col = self.staves[dest_id].col().expect("DEST COL");
    	let contested_addr = CardAddr{stave_id:dest_id, slot_id:card_addr.slot_id};
    	{
    		if let Some(ref card) = self.staves[card_addr.stave_id].card_at(card_addr.slot_id) {
    			if card.pts_for(src_col) <= card.pts_for(dest_col) {
    				println!("NOT DOWNWARD SLIDE");
    				return;
    			}
    			if let Some(ref defender) = self.staves[dest_id].card_at(card_addr.slot_id) {
    				println!("CONTESTED MOVE");
    			} else {
    				println!("EMPTY MOVE");
    			}
    		} else {panic!()}
    ...
    fn forward_card(&mut card_addr: CardAddr) {
    	if let Some(ref card) = self.staves[card_addr.stave_id].card_at(card_addr.slot_id) {
    		match card.owner() {
    			Player::Top => {
    				if card_addr.slot_id < 2
    			},
    			Player::Bottom => {
        ...
        for (stave_id, stave) in self.staves.iter().enumerate() {
        	...
        	for (slot, card) in stave.iter_slots_forward(Player::Top) {
        		if let Some(ref card) = card {
    				...
    				let card_pt = Self::card_point(stave_id, slot, card);
		        	let param = graphics::DrawParam {
		    			dest: card_pt, .. Default::default()
		    		};
    				graphics::draw_ex(ctx, &self.card_mesh, param)?;

    				for &col in [Col::Red, Col::Green, Col::Blue].iter() {
    					graphics::set_color(ctx, darken(col_to_color(Some(col)), 0.4))?;
    					let mut stat_pt = card_pt.clone();
    					Self::offset_pt_for_text(col, &mut stat_pt);
    					let param = graphics::DrawParam {
			    			dest: stat_pt, .. Default::default()
			    		};
    					graphics::draw_ex(ctx, self.get_text_for(card.pts_for(col)), param)?;
